...
├── benchmark.sh
├── contracts/
│   ├── verifier.sol             // Placeholder for device-exported verifier (101KB, commit manually)
...
### Why `--oracle_hash keccak` is Required

Noir circuits, by default, use Poseidon hashing for their transcript. However, Solidity verifiers typically expect Keccak256 hashing. To ensure compatibility between the proof generated by `bb prove` and the Solidity verifier, the `--oracle_hash keccak` flag must be used during both the `bb prove` and `bb write_vk` steps. This ensures that the proof uses Keccak256 transcript hashing, matching the Solidity verifier's expectations for on-chain verification.
